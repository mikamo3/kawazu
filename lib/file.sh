...
#when target does not exist. return 1
get_abs_path() {
  if [[ $# == 0 ]]; then
  ...
  if [[ ! -e $dir ]]; then
    print_error "${FUNCNAME[0]} : $f does not exists"
    return 1
  fi
...
# return resolved symbolic link absolute path
# return 1 when target path does not exist or not symbolic path
get_symlink_abs_path() {
  ...
  if [[ ! -e "$1" ]]; then
    if [[ -L "$1" ]]; then
      print_error "${FUNCNAME[0]} : $1 is broken symbolic link"
    else
      print_error "${FUNCNAME[0]} : $1 does not exists"
    fi
    return 1
  fi

  if [[ ! -L "$1" ]]; then
    print_error "${FUNCNAME[0]} : $1 is not symbolic link"
    return 1
  fi
  ...
  get_abs_path "$(dirname "$target_path")/${target_symlink_path}"
}
...
get_common_path() {
  ...
  for ((i = 0; i < ${#target_path_arr_1[@]} && i < ${#target_path_arr_2[@]}; i++)); do
...
get_rel_path() {
  ...
  while [[ "$common_path" != "$cur_dir" && -n "$cur_dir" ]]; do
    cur_dir=${cur_dir%/*}
    output_path+="../"
  done

  [[ -z "$output_path" ]] && output_path="./"
  [[ "$target_path" != "$common_path" ]] && output_path+=${target_path#$common_path/}
